...
function CartModal({ visible, showModal }) {
    ...
    const cartItems = typeof window !== "undefined" && JSON.parse(localStorage.getItem("userCart")) || [];
    ...
    const totalPriceCounter = (latestItems) => {
        const totalPriceOfALl = latestItems?.reduce((total, item) => (total + (+item.price * +item.quantity)), 0);
    ...
    return (
        <div className="fixed inset-0 z-[200] text-white bg-background bg-opacity-75 backdrop-blur-md flex justify-center items-center" >
            ...
            <div className="hs-overlay-open:opacity-100 hs-overlay-open:duration-500 transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto">
                <div className="flex flex-col bg-background   shadow-sm rounded-xl dark:bg-gray-800 dark: -gray-700 dark:shadow-slate-700/[.7]">
                    <div className="flex justify-between items-center py-3 px-4  -b dark: -gray-700">
                        <h3 className="font-bold text-primary dark:text-white">
                            Total {cartItems.length} {cartItems.length === 1 ? "item" : "items"} in your cart .!
                        </h3>
                        <button onClick={() => showModal(p => !p)} type="button" className=" inline-flex flex-shrink-0 justify-center items-center h-8 w-8 rounded-md text-gray-500 hover:text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-offset-2 focus:ring-offset-white transition-all text-sm dark:focus:ring-gray-700 dark:focus:ring-offset-gray-800" data-hs-overlay="#hs-basic-modal">
                    ...
                    <div className="p-4 overflow-y-auto">
                        {cartItems?.length && cartItems?.map(({ cuisineImg, price, availableAt, isExclusive, name, quantity }) => (
                            <FoodWithDetails key={price + name + availableAt} cuisineImg={cuisineImg} price={price} availableAt={availableAt} isExclusive={isExclusive} name={name} quantity={quantity} setCheck={setCheck} />
                        ))}
                    </div>
                    <div className="flex justify-between pl-7 text-primary">
                        ...
                        {totalPrice > 0 ?
                            <Link href={'/checkout/cartDataOnly'}>
                            ...
                            :
                            <button disabled={true} className="py-3 px-4 flex flex-row items-center justify-center gap-3 bg-primary bg-opacity-10 rounded-md opacity-50 transition-all" >
                                Place order <HiShoppingBag />
                            </button>
